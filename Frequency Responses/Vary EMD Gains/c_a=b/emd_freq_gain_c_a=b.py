...
def freq_response_emd(gain_left, gain_center, gains_right, freqs, num_cycles, t4=True):
    if t4:
        invert = False
        title = 'T4'
    else:
    ...
    for g in range(num_gains):
        ...
        plt.subplot(2,1,1)
        plt.plot(freqs, a_peaks, label=str(gains_right[g]), color='C' + str(g))
        plt.plot(freqs, b_peaks, linestyle='--', color='C'+str(g))
        plt.title('A Frequency Response')
        plt.ylabel('Steady Peak')
        plt.xlabel('Frequency (Hz)')
        plt.subplot(2,1,2)
        plt.plot(freqs, ratios, label=str(gains_right[g]))
...
for i in range(len(gains)):
    ...
    plt.figure()
    freq_response_emd(gains[i], gains[i], gains, freqs, 10, t4=True)
    print('Run %i/%i: T5' % (i + 1, len(gains)))
    plt.figure()
    freq_response_emd(gains[i], gains[i], gains, freqs, 10, t4=False)
