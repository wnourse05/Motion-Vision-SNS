...
def create_net(k, cutoff, params_in, params_bp, params_d_on):
    ...
    reversal_d_on_on = reversal_ex
    g_d_on_on = k*activity_range / (reversal_d_on_on - k*activity_range)

    synapse_d_on_on = NonSpikingSynapse(max_conductance=g_d_on_on, reversal_potential=reversal_d_on_on, e_lo=0.0,
                                          e_hi=activity_range)

    add_lowpass_filter(net, cutoff=cutoff, name='On', invert=False)

    net.add_connection(synapse_d_on_on, 'D On', 'On')

    net.add_output('On', name='Out')
...
def tune_on(cutoff, params_in, params_bp, params_d_on, div):
    f = lambda x : error(x, 1.0, cutoff, params_in, params_bp, params_d_on)
    res = minimize_scalar(f, bounds=(1.0,2.0), method='bounded')

    k_final = res.x

    g_s, rev_s = synapse_target(0.0, activity_range)
    g = {'direct': k_final*activity_range / (reversal_ex - k_final*activity_range),
        'enhance': div - 1,
        'suppress': g_s}
    reversal = {'direct': reversal_ex,
                'enhance': 0.0,
                'suppress': rev_s}
