...
def freq_response_emd(gains_left, gain_center, gain_right, freqs, num_cycles, t4=True):
    if t4:
        invert = False
        title = 'T4'
    else:
    ...
    for g in range(num_gains):
        ...
        if t4:
            model, net = gen_motion_vision((7, 7), k_mi1=gain_center, k_mi9=gains_left[g], k_ct1on=gain_right,
                                           output_t4a=True, output_t4b=True)
        else:
            model, net = gen_motion_vision((7, 7), k_tm1=gain_center, k_tm9=gains_left[g], k_ct1off=gain_right,
    ...
    plt.subplot(2,1,2)
    plt.legend()
    plt.suptitle(title +': b=c=' + str(gain_center))
...
for i in range(len(gains)):
    ...
    plt.figure()
    freq_response_emd(gains, gains[i], gains[i], freqs, 10, t4=True)
    print('Run %i/%i: T5' % (i + 1, len(gains)))
    plt.figure()
    freq_response_emd(gains, gains[i], gains[i], freqs, 10, t4=False)
