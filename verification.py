from motion_vision_net import SimpleOn
...
def run_sample(sample, net: nn.Module):
    ...
    """
    state_input, state_bo_input, state_bo_fast, state_bo_slow, state_bo_output, state_lowpass, state_enhance_on, state_direct_on, state_suppress_on, state_ccw_on, state_cw_on = net.init()
    net.setup()
    # net.zero_grad()
    num_sub_steps = 13
    warmup = 200
    ...
    for i in range(warmup):  # warmup
        state_input, state_bo_input, state_bo_fast, state_bo_slow, state_bo_output, state_lowpass, state_enhance_on, state_direct_on, state_suppress_on, state_ccw_on, state_cw_on = net(
        ...
        bo_input[:, step] = state_bo_input.flatten()
        bo_fast[:, step] = state_bo_fast.flatten()
        bo_slow[:, step] = state_bo_slow.flatten()
        bo_out[:, step] = state_bo_output.flatten()
        low[:, step] = state_lowpass.flatten()
        enhance[:, step] = state_enhance_on.flatten()
        direct[:, step] = state_direct_on.flatten()
        suppress[:, step] = state_suppress_on.flatten()
        cw[:, step] = state_cw_on.flatten()
        ccw[:, step] = state_ccw_on.flatten()
        step += 1
    ...
    for i in range(sample.shape[0]):
        for j in range(num_sub_steps):
            ...
            state_input, state_bo_input, state_bo_fast, state_bo_slow, state_bo_output, state_lowpass, state_enhance_on, state_direct_on, state_suppress_on, state_ccw_on, state_cw_on = net(sample[i, :, :], state_input, state_bo_input, state_bo_fast, state_bo_slow, state_bo_output, state_lowpass, state_enhance_on, state_direct_on, state_suppress_on, state_ccw_on, state_cw_on)
    ...
    return inp, bo_input, bo_fast, bo_slow, bo_out, low, enhance, direct, suppress, ccw, cw
...
net = SimpleOn(params['dt'], [24, 64], device=params['device'])
if plot:
    plt.figure(1)
    plt.suptitle('Input')
    plt.figure(2)
...
with torch.no_grad():
    ...
    for i, data in enumerate(loader_testing):
        ...
        state_input, state_bo_input, state_bo_fast, state_bo_slow, state_bo_output, state_lowpass, state_enhance_on, state_direct_on, state_suppress_on, state_ccw_on, state_cw_on = run_sample(frames, net)
        ...
        if plot:
            ...
            plt.plot(state_enhance_on[200,:], label='Enhance')
            plt.plot(state_direct_on[200,:], label='Direct')
            plt.plot(state_suppress_on[200,:], label='Suppress')
